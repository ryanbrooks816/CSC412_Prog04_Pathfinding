...
void findCheapestPath(Graph &graph, int start, int dest, std::string scrapFolderPath)
{
    ...
    {
        std::cerr << "No valid paths found in the graph." << std::endl;
        return;
    }
    ...
    testValidPaths(validPaths, "tree_valid.txt");
#endif
...
#ifdef DEBUG
    testValidPaths(allPaths, "tree.txt");
#endif
...
void testValidPaths(std::vector<std::vector<int>> &paths, std::string filename)
{
    ...
    std::ofstream outFileValid(filename);
